...
public interface DoctorReportRepository extends JpaRepository<DoctorReport, Long> {
    ...
    @Query("select r from DoctorReport r where r.pCode = :pCode and r.userId = :userId")
    Optional<DoctorReport> findByPCodeAndUserId(@Param("pCode") Integer pCode, @Param("userId") String userId);
}
